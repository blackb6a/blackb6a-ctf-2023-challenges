...
MATRIX_FILE = 'mt19937_64_matrix_skip_40000'
length = 19968
skip = 40000
...
def get_state0(leak, state):
    ...
    seq1 = [rng.getrandbits(1) for _ in range(length)]
    rng.skip_state(skip)
    guess_result = [rng.getrandbits(1) ^ b for b in seq1]
...
def get_leak(rng = None):
    ...
    seq1 = [rng.getrandbits(1) for _ in range(length)]
    to_predict = [rng.getrandbits(1) for _ in range(skip)]
    seq2 = [rng.getrandbits(1) ^ b for b in seq1]
    return seq2, prefix, to_predict
...
def main():
    ...
    predict = [rng.getrandbits(1) for _ in range(skip)]
