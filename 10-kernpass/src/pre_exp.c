...
# 13 "/usr/include/linux/userfaultfd.h" 3 4
...
# 14 "/usr/include/linux/userfaultfd.h" 2 3 4
...
# 28 "/usr/include/x86_64-linux-gnu/sys/poll.h" 2 3 4
...
int add_password(unsigned int index, unsigned int size, char *pw){
    ...
    req.size = size;
    req.password = pw;
    int ret = ioctl(fd, 0x13370001, &req);
    if (ret < 0) die("Add");
...
int check_password(unsigned int index, char *pw){
    ...
    req.size = 0;
    req.password = pw;
    int ret = ioctl(fd, 0x13370002, &req);
...
int edit_password(unsigned int index, char *pw){
    ...
    req.size = 0;
    req.password = pw;
    int ret = ioctl(fd, 0x13370003, &req);
    if (ret < 0) die("EDIT");
...
int delete_password(unsigned int index){
    ...
    req.size = 0;
    req.password = 0;
    int ret = ioctl(fd, 0x13370004, &req);
    if (ret < 0) die("RESET");
...
# 158 "exp.c"
   msg.event == 
# 158 "exp.c" 3 4
   0x12) ? 1 : 0), __extension__ ({ if (
# 158 "exp.c"
   msg.event == 
# 158 "exp.c" 3 4
   0x12) ; else __assert_fail (
# 158 "exp.c"
   "msg.event == UFFD_EVENT_PAGEFAULT"
# 158 "exp.c" 3 4
   , "exp.c", 158, __extension__ __PRETTY_FUNCTION__); }))
# 158 "exp.c"
                                             ;
    ...
    if (ioctl(uffd, 
# 175 "exp.c" 3 4
                   (((2U|1U) << (((0 +8)+8)+14)) | (((0xAA)) << (0 +8)) | ((((0x03))) << 0) | ((((sizeof(struct uffdio_copy)))) << ((0 +8)+8)))
# 175 "exp.c"
                              , &copy) == -1) die("ioctl(UFFDIO_COPY)");
  }
...
# 191 "exp.c"
  ...
  if (ioctl(uffd, 
# 193 "exp.c" 3 4
                 (((2U|1U) << (((0 +8)+8)+14)) | (((0xAA)) << (0 +8)) | ((((0x3F))) << 0) | ((((sizeof(struct uffdio_api)))) << ((0 +8)+8)))
# 193 "exp.c"
                           , &uffdio_api) == -1)
...
# 199 "exp.c"
  ...
  if (ioctl(uffd, 
# 200 "exp.c" 3 4
                 (((2U|1U) << (((0 +8)+8)+14)) | (((0xAA)) << (0 +8)) | ((((0x00))) << 0) | ((((sizeof(struct uffdio_register)))) << ((0 +8)+8)))
# 200 "exp.c"
                                , &uffdio_register) == -1)
    die("UFFDIO_REGISTER");
