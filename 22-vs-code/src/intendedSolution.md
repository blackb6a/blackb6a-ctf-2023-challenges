...
- A key insight is that $\overrightarrow{text pixel}-\overrightarrow{background} = k * (\overrightarrow{text color}-\overrightarrow{background})$ , which means $\hat{\overrightarrow{text pixel}-\overrightarrow{background}}$ is dependent on true text color only
...
![A tiny helper png file showing how letters in the ascii range is rendered with white text color under a black background](./_resources/letters.png)
...
# it's okay to not have all colors
...
assert cipher.encrypt(inside).hex() == "?03dffdf6?de1a?4143??2e1303fd3?????f11d???d???ca6?4a4bf621d6c1e44c?a0f3e?cfc?3dbedfb?4f00aa?bcde??6d1?"
...
- The code also details how the flag is encrypted with a key composed of 2 halves:
  - The first half uses 4 different syntax highlighting colors used:
...
- There are only *4* unknown letters in the encoded flag, which can be readily bruteforced
  - ~~they are really digits and not `B` or `F`, which haven't appeared in the program text (the bruteforcing is still trivial with them considered but this challenge is already hard enough)~~